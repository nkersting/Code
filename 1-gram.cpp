...
int main() 
{
    ...
    case 1: cout << "Applying Google 1-grams";
        ...
        totalwords = 6500000;  // fix for now
...
/////////////////////////////////////////////////////////////////////////////
   ...
   ifstream summ_file;
    summ_file.open ("summary.txt"); 


    unsigned long allwords=0;
    unsigned long allbooks=0;   
    unsigned long allpages=0;
    unsigned long words=0;
    unsigned long books=0;   
    unsigned long pages=0;
    int year;

    if (summ_file.is_open())
    {
     while ( !summ_file.eof())
    {
      summ_file >> year >> words >> books >> pages;
      allwords += words;
      allpages += pages;
      allbooks += books;
      }
    ...
    outfile << endl << endl << "Words Pages Books " << allwords << " " << allpages << " " << allbooks << endl;
  ...
  outfile.close();
	...
	//cout << "melon is " << hashval(melon) << endl;

    //cout << "lemon is  " << hashval(lemon) << endl;
*/
    ...
    ***************************************************
    ...
    Next, read in a word compute the rank.
    With ~43000 entries in the hashtable, should need about
    log(43000) ~ 15.4 binary queries.
    If not found, rank = 0.
    ...
    ****************************************************/
    ...
    while (length != 0)
    {
        ...
        const char *hashstring = userstring.c_str(); 
        unsigned long testkey = hashval(hashstring);
        while (testkey != freqs[index][0]){

            if(testkey < freqs[index][0]){
                index_hi = index;                    
            }
            else {
                index_lo = index;
        ...
        cout << "Rank is " << freqs[index][1] << endl;
    }
    ...
    if (myfile.is_open())
  {
    while ( !myfile.eof())
    {
      ...
      outfile << val  << " " << rank << " " << -1.0*log(1.0*rank/allwords) << endl;
    }
    myfile.close();
    }
  ...
  */
  ...
  /*
     ...
     while (true)
      ...
      cout << "Enter String \n" << endl;
      getline( cin, ss );
      cout << "Hash value is " << hashval(ss.c_str()) << endl;
      cout << "detropy is " << hashval(ss.c_str()) << endl;
      if (cin.get() == '\n') break;
  
 //     cout << "melon is " << ss << endl;
...
/*
  ...
  // some old code to read in Project Gutenberg words
    
    ifstream myfile;
    myfile.open ("gp-raw.txt");  
...
//
  ...
  ifstream myfile;
    myfile.open ("googlebooks-eng-all-1gram-20090715-4.csv");  
...
/*
    ...
    if (myfile.is_open())
  {
    while ( myfile.good() )
    {
      myfile >> wrd >> freq;
      total += freq;
      outfile << hashval(wrd.c_str())  << " " << wrd << " " << (int)freq << endl;
    }
    myfile.close();
  }
