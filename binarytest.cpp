////////////////////////////////
// This is meant to be compared to findtest.cpp, which implements
// std::find instead of std::binary_search. 
// Something is wrong: the search takes an extremely long time
#include "groups.h"
...
const char* DATADIR = "/home/nkersting/quantumrepoire.com/public5/";
...
double findMaxOverlap(const set<int>& keyListA, const set<int>& keyListB)
{
  ...
  if (keyListA.size() <= keyListB.size())
  {
     ...
     {
        if (binary_search(keyListB.begin(),keyListB.end(),*it)) commonKeys.insert(*it);
     }
  ...
  else
  {
     ...
     {
        if (binary_search(keyListA.begin(),keyListA.end(),*it)) commonKeys.insert(*it);
     }
...
int main(int argc, char* argv[])
{
   ...
   std::map<string, set<int> > keyList;
   readFilesIntoMap(keyList);
   ...
   int count = 0;
   set<int> baseList;
   for(map<string, set<int> >::iterator it = keyList.begin();
       it != keyList.end(); it++)
   {

      if (count++ == 0) baseList = it->second;
      std::cout << findMaxOverlap(it->second, baseList) << endl;
