...
void WriteRepoirePage()
{
  ...
  outfile << "<html xmlns=\"http://www.w3.org/1999/xhtml\">\r\n";
   ...
   outfile << "  <p>What  you see below is a <a href=\"http://en.wikipedia.org/wiki/"
           << "Force-based_algorithms_(graph_drawing)\">\r\n"; 
   outfile << "force-based network layout</a> where messages"
           << " are like charges"
           << " and links between messages act like springs pulling them together. </p>\r\n"; 

   outfile << "<ul> <li> Hovering over a submission shows the Public Label + timestamp, semantic size, and links to other submissions. </li>\r\n";
   outfile << " <li> Clicking on a submission will bring up another window where you can try a quantitative"
           << " match to that submission</li>\r\n"; 
   outfile << "      <li>There is also a search form below were you can search for a submission"
           << " by its Public Label:\r\n";
   outfile << "  matching results will be <b>blackened</b> in the plot.</li></ul>\r\n"; 
   ...
   outfile << "  <option value=\"scientific.json\">Scientific Titles</option>\r\n";
   outfile << "  <option value=\"hr.json\">HR Issues</option>\r\n";
   outfile << " </select>\r\n";
...
///////////////////////////////////////////
bool IsNotFluff(std::string input) 
{
   return (input != "in"          // eliminate words which probably don't add content
           && input != "at" 
           && input != "of"
           && input != "on"
           && input != "to"
...
int main(int argc, char* argv[])
{
  ...
  {
     string cleanedString(cleanString(*it));
     if (IsNotFluff(cleanedString)) new_input_words.push_back(cleanedString);   
  }

  string newUserID = cleanString(userID);
  std::replace(newUserID.begin(), newUserID.end(), '"', '_');  // remove all quotes since this will throw off JSON
  ...
  WriteLineJSON();    // Write the JSON file used to make lines 
