...
const char* DATADIR = "/home/nkersting/quantumrepoire.com/public4/";
...
int main(int argc, char* argv[])
{
       ...
       it != keyList.end(); it++)
   {
      ...
      for (set<int>::iterator iter = (it->second).begin();
           iter != (it->second).end(); ++iter) 
      {
         if (*iter > 0 && *iter < KEY_RANGE)
         {
            spectrum[((*iter)*FREQUENCY_BINS)/KEY_RANGE]++;
         }
         else
         {
            spectrum[(int)(log(abs(*iter)))]++;    // keys from non-dict words are collected in the lowest bins
         }
      }
