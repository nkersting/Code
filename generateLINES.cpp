...
const double MIN_OVERLAP = 0.0001; 
...
const char* LINEFILE = "/home/nkersting/quantumrepoire.com/lines.json"; 
...
// first write bubble plot
  ...
  cout << "<script src=\"../lines.json\" type=\"text/javascript\"></script>\r\n";
...
//void WriteLineJSON(vector<vector<double> >& connectionLines)
void WriteLineJSON()
{
...
void WriteLineJSON(vector<vector<double> >& connectionLines)
{
  ...
  for (vector<vector<double> >::iterator it = connectionLines.begin();
       it != connectionLines.end(); ++it)
  {
     if (it != connectionLines.begin()) outfile << "," << endl;
     outfile << "{" <<
        "\"x1\": \"" << (*it).at(0)  << "\"," << endl <<
        "\"y1\": \"" << (*it).at(1)  << "\"," << endl <<
        "\"x2\":" << (*it).at(2) << "," << endl << 
        "\"y2\":" << (*it).at(3) << "," << endl << 
        "\"overlap\":" << (*it).at(4) <<  "}" << endl ; 
  }
  outfile << "]} ]} ] }";
...
/*
      ...
      string linefilename(DATADIR_LINES);
      linefile.open (linefilename.c_str(), fstream::out | fstream::trunc);
*/

   ofstream linefile;
   linefile.open(DATADIR_LINES);   
        ...
        itx != keyLists.end(); ++itx)
   {
      ...
      for (map<int, set<int> >::iterator ity = keyLists.begin();                                                    
      ...
      { 
         ...
         for (set<int>::iterator it2 = (ity->second).begin();
              it2 != (ity->second).end(); ++it2) 
         {
            int Key = *it2;
            if (Key > 0 && Key < KEY_RANGE)
            {
         ...
         {
            linefile << x1 << " " << y1 << " " << x2 << " " << y2 << " " << overlap << " " << xcount << " " << ycount << endl;
         }
      }
