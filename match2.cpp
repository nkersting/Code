...
const char* DATADIR = "/home/nkersting/quantumrepoire.com/public2/";
...
void populateKeyMapAndTotMap(std::map<int, int> &TotMap, 
      ...
      for (int j = i+1; j < input_words.size(); j++)
	{
	  ...
	  for (vector<int>::iterator it1 = keys1.begin(); it1 != keys1.end(); it1++)
	      ...
	      for (vector<int>::iterator it2 = keys2.begin(); it2 != keys2.end(); it2++)
		{
		  ...
		  TotMap[(*it1 + *it2)] = (keys1.size())*(keys2.size());
		}  
	    }
		      ...
		      vector <int>& keylist)
{
  ...
  for (vector<int>::iterator it = keylist.begin(); it != keylist.end(); it++)
	...
	{
	  for (vector<string>::iterator it2 = (KeyMap[*it]).begin(); 
	       it2 != (KeyMap[*it]).end(); it2++)
	    {
	      if (MatchMap.find(*it2) == MatchMap.end())
		{
		  MatchMap[*it2] = 1.0/TotMap[*it];
		}
	      else
		{
		  MatchMap[*it2]+= 1.0/TotMap[*it];
		      ...
		      float normalization_factor)
{
  ...
  for (std::map<float, vector <std::string> >::iterator iter = SortedMap.begin(); 
       iter != SortedMap.end(); iter++)
    {
      cout << "<tr>\r\n";
      cout << "<td>\r\n";
      cout << -1.0*(iter->first)/normalization_factor << ": ";
      int count = 0;
...
void  WriteMatchPage(std::map<std::string, float >& MatchMap, float normalization_factor, string input_file)
{
  ...
  cout << "<p>Please copy the filename in the \"Contributions\" column of the previous page and paste in \"Target File\" below." <<
    "Then enter the text you would like to match this against. Results are rated from \"N\" (at least N exact word matches) to \"0.0\" (no match)." <<
  ...
  cout << "<tr>\r\n";
  cout << "<form action=\"/cgi-bin/match2.cgi\" method=\"post\" name=\"TestForm\" >\r\n";
  ...
  print_SortedMap(SortedMap, MatchMap, normalization_factor);
...
int main(int argc, char* argv[])
{
  ...
  int num_input_words = new_input_words.size();   // used for normalization later
  float normalization_factor = (num_input_words-1);
  ...
  if (new_input_words.size() < 2)
  ...
  else
    {
      ...
      std::map<int, vector <std::string> > KeyMap;
      std::map<int, int> TotMap;
      populateKeyMapAndTotMap(TotMap, KeyMap, WordMap, new_input_words); 
      // generate keys for this sentence



      // populate a match list with weights for each word
      std::map<std::string, float > MatchMap;
      make_MatchMap(MatchMap,KeyMap,TotMap,keylist);


      WriteMatchPage(MatchMap, normalization_factor, newUserFILE);  
