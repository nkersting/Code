...
const char* DATADIR = "D:\\website\\public\\";

const char* GROUPSFILE = "D:\\website\\groups.json";
...
bool CompareToAddToGroup(string key, std::map<string, set<int> >& keyListsCopy, vector <RepoireGroup>& Groups, float threshold)
{
   ...
   for (vector <RepoireGroup>::iterator group = Groups.begin(); group != Groups.end(); group++)
   {
      ...
      if (firstPercent >= threshold  && secondPercent >= threshold)  // added to first group that matches
      {
...
void WritePage(vector <RepoireGroup> Groups, float threshold)
{
	...
	cout << "<title>Repoire Social Networking Results</title>\r\n";
	cout << "<script type=\"text/javascript\" src=\"../d3.v2.js\"></script>\r\n";
    cout << "<link type=\"text/css\" rel=\"stylesheet\" href=\"../bubble.css\"/>\r\n";
	cout << "</head>\r\n";
	cout << "<body>\r\n";
	cout << "<p>Submission received!</p>\r\n";
	cout << "<p>An encoding of your contribution (which only you know) has been added to the server in the " <<
		"<a href=\"../public\">public/ directory</a>, alongside all the other contributions thus far uploaded." <<
		"You can in fact download any of these to your machine to try individual matches with the C++ source (please email me)" <<
		"together with the <a href=\"../word-synsets.txt\"> dictionary </a>" <<
		"and <a href=\"../synsets.txt\">thesaurus</a>," <<
		"or click on the word \"MATCH\" below (again confidential) to try a match on the server.</p>\r\n";
	cout << "<p>&nbsp;</p>\r\n";

	cout << "<p><a href=\"../match.html\" title=\"Match Form\" target=\"_new\">MATCH</a></p>\r\n";

	cout << "<form action=\"/cgi-bin/group\" method=\"post\" name=\"GroupForm\" >\r\n";
	cout << "<p>All contributions  can currently be organized into the following groups, based on a threshold commonality percentage of \r\n";
    ...
    cout << "<p>&nbsp;</p>\r\n";
	cout << "<p align=\"center\"><span class=\"style1 style5 style4\"><a href=\"../Repoire.html\" target=\"_self\">back to Repoire</a></span></p>\r\n";
    cout << "</body>\r\n";
    cout << "</html>\r\n";
...
int _tmain(int argc, _TCHAR* argv[])
{
 ...
 // cout << (*itr) << endl;
  ...
  while (getline(read_ss, read_s, '&'))
  {
	  ...
	  {
		  userID.append(read_s.substr(11,read_s.size() - 11));
	  }
  }
 ...
 ////////////////////////////////////
  ...
  else
  {
    ...
    populateKeyMap(KeyMap, WordMap, input_words);    // generate keys for this sentence
